...
ring_simplify.
replace (n- 2 + 2)%nat with (n-0)%nat.
lia.
...
ring_simplify.
replace (n- 2 + 2)%nat with (n-0)%nat; auto; lia.

subst n.
simpl (length (a :: l)).
replace (S (length l)) with (length l +1)%nat; lia.
subst n.
simpl (length (a :: l)).
replace (S (length l)) with (length l +1)%nat; auto.
apply symmetry.
