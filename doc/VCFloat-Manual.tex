...
forward error (1) does not expand into a nice multinomial
that is tractable for the \lstinline{prune} tactic, and (2) is 
so large (as a meausure of the number of operations after 
rounding) that applying the \lstinline{field_simplify+interval} 
...
VCFloat's \lstinline{prune} tactic won't simplify the 
...
where the total error is now bounded by $e3 + e2 + e1$. 

Note that the subexpressions in the above subgoals are smaller 
and 
...
in \lstinline{error_rewrites}, the \lstinline{prune} tactic can 
